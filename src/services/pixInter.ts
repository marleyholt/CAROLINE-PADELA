...
import { ConfiguracaoInter } from '../types';
...
export function gerarPayloadPixBacen(
  ...
  valor: number,
  txId: string = 'INTER50SINAL'
): string {
  // Limpar e normalizar
  const cleanKey = chavePix.trim();
  const cleanName = (nomeTitular || 'CONSULTORIO MASSOTERAPIA')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toUpperCase()
    .slice(0, 25);
  const cleanCity = (cidadeTitular || 'SAO PAULO')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toUpperCase()
    .slice(0, 15);
  const cleanTxId = txId.replace(/[^a-zA-Z0-9]/g, '').slice(0, 25) || 'INTER50';
...
export interface InterCobrancaPixResult {
  txid: string;
...
export async function emitirCobrancaSinalBancoInter(
  valorSinal: number,
  pacienteNome: string,
  procedimentoNome: string,
  configInter: ConfiguracaoInter
): Promise<InterCobrancaPixResult> {
  const cleanName = pacienteNome.replace(/\s+/g, '').slice(0, 6).toUpperCase();
  const randomSuffix = Math.floor(1000 + Math.random() * 9000);
  const txid = `INTER${cleanName}${randomSuffix}`;

  const payloadPix = gerarPayloadPixBacen(
    configInter.chavePix || 'consultorio.massoterapia@inter.com.br',
    configInter.nomeTitular || 'CONSULTORIO MASSOTERAPIA',
    configInter.cidadeTitular || 'SAO PAULO',
    valorSinal,
    txid
  );
